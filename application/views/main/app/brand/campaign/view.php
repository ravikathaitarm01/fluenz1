 ...
 */
$data['_this'] = array(
	'page-class' => 'admin-campaign-view',
	'page-js' => array(
		'main/js/app/brand/campaign/view.js',
	)
);
?>
...
<div class="wrapper">
	...
	<div class="col-sm-12">
		<div class="panel panel-primary panel-body">
			<?php if (in_array($data['campaign']['state'], array('pending', 'active'))): ?>
				<form class="col-sm-1" method="post" action="<?=$Url::base('brand/campaign/reject')?>">
		...
		<section class="panel bordered  post-comments">
			<div>
				...
				<div class="panel-footer">
					<form role="form" class="form-horizontal" method="post" action="<?=$Url::base('brand/campaign/comment/admin')?>">
						<input type="hidden" name="id" value="<?=$data['campaign']['_id']?>" />
	...
	<div class="col-sm-6">
		<h3>Brand - Influencer</h3>
		<section class="panel bordered  post-comments">
			<?php foreach ($MongoDoc::get($data['campaign'], 'comments.brand_influencer', array()) as $k=>$comments): ?>
				<div id="conversation-<?=$k?>">
					<div class="media p15">
						<?php foreach ($comments as $c): ?>
							<div class="media">
								<div class="comment">
									<div class="pull-right">
										<span class="label label-<?=$c['user'] == 'brand'? 'info': 'color'?>"><?=$c['user']?></span>
									</div>
									<div class="comment-author h5 no-m">
					...
					<div class="panel-footer">
						<form role="form" class="form-horizontal" method="post" action="<?=$Url::base('brand/campaign/comment/influencer')?>">
							<input type="hidden" name="id" value="<?=$data['campaign']['_id']?>" />

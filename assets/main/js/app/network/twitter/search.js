$(function () {
    ...
    var $twitterSearches = $('.x-network-marker-twitter-search:first').parent();
    var $urls = $('#x-data-network-url');

    $twitterSearches.on('click', '.x-twitter-item-favorite', function(e){
    ...
    $twitterSearches.on('click', '.x-twitter-item-retweet', function(e){
        var $item = $(this).closest('.x-network-item');
        var id = $item.data('id');
        var $spinner = $item.find('.spinner:eq(0)');
    ...
    $twitterSearches.on('change', 'select.x-network-item-predef', function(){
        var $e = $(this).parent().parent().find('.x-network-item-comment');
        $e.val($e.val().match(/\S+/) + ' ' + decodeURI($(this).val()));
    });
    $twitterSearches.on('click', '.x-twitter-item-show-comment', function(){
        console.log($(this).parent().parent());
        $(this).parent().parent().find('> .media').toggle('slow');
    });

    var $popoverComment = null;
    $twitterSearches.on('click', 'a.x-twitter-item-comment', function(e){
        var $item = $(this).closest('.x-network-item');
        var id = $item.data('id');
        var $spinner = $item.find('.spinner:eq(0)');
    ...
    $twitterSearches.on('click', 'a.x-network-item-query', function(e){
        var $item = $(this).closest('.x-network-item');
        var $spinner = $item.find('.spinner:eq(0)');

$(function () {
    ...
    var $facebookPosts = $('.x-network-marker-facebook-conversation:first').parent();
    ...
    $facebookPosts.on('click', 'a.x-facebook-item-comment', function(e){
        ...
        $popoverComment = $(this).popover({
            ...
            $p.find('.x-network-item-comment-post').click(function(e) {
                ...
                $spinner.show();
                $.post($urls.data('networkItemFacebookAction'), {action: 'message', id: id, message: message}, function(d) {
                    try {
                        d = $.parseJSON(d);
                        if (d['success']) {
